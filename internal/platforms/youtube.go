...
func (p *YouTubePlatform) handleCombined(rawURL, videoID string) ([]*state.Track, error) {
	...
	if pErr == nil {
		gologging.WarnF("[YouTube] Failed to fetch video %s: %v", videoID, vErr)
		return pTracks, nil
	}
...
// Download uses Shruti API — same as Python repo
func (p *YouTubePlatform) Download(
	...
	if track.Video {
		return p.downloadFromShruti(track.URL, "video", track.ID+".mp4", 600*time.Second)
	}
	return p.downloadFromShruti(track.URL, "audio", track.ID+".mp3", 300*time.Second)
}

// VideoSearch uses InnerTube — same as original fallenapi
...
// --- Shruti API: only for downloading ---

func (p *YouTubePlatform) downloadFromShruti(link, fileType, fileName string, timeout time.Duration) (string, error) {
	...
	if resp.StatusCode != 200 {
		return "", fmt.Errorf("shruti api returned status %d", resp.StatusCode)
	}
...
// --- InnerTube: search & metadata (same as original) ---
